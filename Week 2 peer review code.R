...
newdat <- dat %>% select(educ, edloan, employ, CC18_308a)
newdat
...
#### Write your code below:
newdat$trump_approve_disapprove <- recode(newdat$CC18_308a,'1'='1', '2'='1', '3'='0', '4'='0')
newdat
...
dat %>% filter(employ == 1 & marstat == 1) %>%
  summarise("Mean Importance of Religion"=mean(pew_religimp, na.rm=TRUE), "Median Importance of Religion"=median(pew_religimp, na.rm=TRUE))
