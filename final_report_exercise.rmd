...
```{r,problem4}
boxplot(dat$faminc_new,data=dat, main="Laura H.", xlab="Family Income")
```
...
dat2 <- dat %>% filter(gender == 1, region == 2 | region == 1, marstat == 1, newsint == 1)
str(dat2)
```
...
dat2 <- dat %>% filter(gender == 1, region == 2 | region == 1, marstat == 1, newsint == 1)
kable(table(dat2$investor),align="l")
``` 
...
datx <- dat %>% mutate(tax_prep = paste0(CC18_325d,CC18_325a), 
                       tax_scale = recode(tax_prep,'12'=0, '21'=1, '11'=2)) %>%
        select(tax_scale,everything(),-tax_prep)
head(datx, n=20)
```
...
kable(table(datx$tax_scale),align="l")
```
...
```{r problem9}
dat %>% group_by(region) %>%
        summarize("Average Trump Approval"=mean(CC18_308a))
```
...
```{r problem10}
dat %>% filter(investor == 2 & faminc_new %in% 5:10) %>%
        summarize('Mean'=mean(pew_religimp,na.rm=TRUE), 
                  'Median'=median(pew_religimp,na.rm=TRUE),
                  'Standard Deviation'=sd(pew_religimp,na.rm=TRUE))
```
...
```{r problem11}
tt <- dat %>% summarize('Mean'=mean(faminc_new,na.rm=TRUE), 
                  'Median'=median(faminc_new,na.rm=TRUE),
                  'Standard Deviation'=sd(faminc_new,na.rm=TRUE))
kable(tt,label="Summary Stats for Family Income")
